...
6. Add a **Warning action** (notify, TTS, script, etc.).
7. Save.
...
| Situation | Result |
|-----------|--------|
| Remaining ≤ threshold and helper is **off** | Runs your warning action, then turns the helper **on** |
| Remaining still low and helper is **on** | No repeat warn |
| Remaining ≥ threshold + margin (or strictly above threshold if margin is 0) | Turns helper **off** (rearmed) |
